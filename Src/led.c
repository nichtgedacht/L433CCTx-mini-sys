...
void led_trans_vals(void)
{
    ...
    for (i = 0; i < NR_LEDS; i++) // each LED
    {
        ...
        {
            aCCValue_Buffer[k++] = led_val[i][j] & 1 << 7 ? 64 : 32; // each bit
            aCCValue_Buffer[k++] = led_val[i][j] & 1 << 6 ? 64 : 32;
            aCCValue_Buffer[k++] = led_val[i][j] & 1 << 5 ? 64 : 32;
            aCCValue_Buffer[k++] = led_val[i][j] & 1 << 4 ? 64 : 32;
            aCCValue_Buffer[k++] = led_val[i][j] & 1 << 3 ? 64 : 32;
            aCCValue_Buffer[k++] = led_val[i][j] & 1 << 2 ? 64 : 32;
            aCCValue_Buffer[k++] = led_val[i][j] & 1 << 1 ? 64 : 32;
            aCCValue_Buffer[k++] = led_val[i][j] & 1 ? 60 : 32;
        }
    }
    for (i = 0; i < 8; i++)
    {
